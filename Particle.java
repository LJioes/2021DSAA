...
//    }
    ...
    public void move(double dt) {
        rx += vx * dt+1/2*ax*dt*dt;
        ry += vy * dt+1/2*ay*dt*dt;
    }
     ...
     * particle, assuming no interesting collisions.
     *
    ...
    public void acceleration(Particle[] Particles, double G){
        this.ax=0;
        this.ay=0;

        for(int i=0;i<Particles.length;i++){
            double s2=(Particles[i].rx-this.rx)*(Particles[i].rx-this.rx)+(Particles[i].ry-this.ry)*(Particles[i].rx-this.ry);
            if((this.radius+Particles[i].radius)*(this.radius+Particles[i].radius)<s2) {
                this.ax += G * Particles[i].mass * (Particles[i].rx-this.rx) / Math.sqrt(s2)/s2;
                this.ay += G * Particles[i].mass * (Particles[i].ry-this.ry) / Math.sqrt(s2)/s2;
            }
        }
    }
